...
class Logic(QObject):
    ...
    @pyqtSlot(int)
    def inSteeringWhilePosition(self, position: int) -> None:
        logging.debug(f"[Logic.inSteeringWhilePosition] New position = {position}")
        value = int(position * self.config.steeringWheelEncoderScaling)
        level = DashboardLevel.ok
